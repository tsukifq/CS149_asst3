...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    // cudaHostRegister(xarray, sizeof(float) * N, cudaHostRegisterDefault);
    // cudaHostRegister(yarray, sizeof(float) * N, cudaHostRegisterDefault);
    // cudaHostRegister(resultarray, sizeof(float) * N, cudaHostRegisterDefault);

    // cudaStream_t stream;
    // cudaStreamCreate(&stream);
    // cudaMallocAsync(&device_x, sizeof(float) * N, stream);
    // cudaMallocAsync(&device_y, sizeof(float) * N, stream);
    // cudaMallocAsync(&device_result, sizeof(float) * N, stream);
    ...
    // cudaHostUnregister(xarray);
    // cudaHostUnregister(yarray);
    // cudaHostUnregister(resultarray);
